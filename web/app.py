...
job_semaphore = threading.BoundedSemaphore(settings.max_concurrent_jobs)
...
def _validate_request(body: dict):
    ...
    if end - start + 1 > settings.max_ports: return None, None, None, None, f"Port range exceeds configured limit of {settings.max_ports} ports"
    targets = list(dict.fromkeys(t.strip() for t in targets_raw.split(",") if t.strip()))
    if len(targets) > settings.max_targets: return None, None, None, None, f"Too many targets; maximum is {settings.max_targets}"
...
def start_scan():
    ...
    def run():
        ...
        if not acquired:
            with jobs_lock:
                jobs[job_id]["status"] = "error"; jobs[job_id]["error"] = "Server is busy; try again shortly"
            return
        try:
            ...
            with jobs_lock:
                ...
                jobs[job_id]["status"] = "done"; jobs[job_id]["duration"] = elapsed
                payload = {"schema_version": "1.0", "scan_time": jobs[job_id]["started_at"], "duration": f"{elapsed}s", "scan_type": scan_type.upper(), "port_range": jobs[job_id]["ports"], "total_open": jobs[job_id]["total_open"], "targets": jobs[job_id]["results"]}
                completed = dict(jobs[job_id])
            history.save(completed)
            report_dir = ROOT / settings.reports_dir; report_dir.mkdir(exist_ok=True)
...
if __name__ == "__main__": app.run(host=settings.host, port=settings.port, debug=False)
